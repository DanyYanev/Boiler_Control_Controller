...
const int PIN_LED = 14; // D4 on NodeMCU and WeMos. Controls the onboard LED.
...
//  pinMode(TRIGGER_PIN2, INPUT_PULLUP);
  ...
  if (WiFi.SSID() == "") {
    ...
    WiFi.mode(WIFI_STA); // Force al modo de estación porque si el dispositivo estaba apagado mientras estaba en el modo de punto de acceso, se iniciará la próxima vez en el modo de punto de acceso.
...
void loop() {
  ...
  if ( (digitalRead(TRIGGER_PIN2) == HIGH) || (initialConfig)) {
    ...
    WiFiManagerParameter p_ipStatic("ipstatic", "IP Adress", ipStatic, 15);
...
//    WiFiManagerParameter p_pinScl("pinscl", "I2C SCL pin", convertedValue, 3);
    ...
    wifiManager.addParameter(&p_ipStatic);
...
//    wifiManager.addParameter(&p_pinScl);  
    ...
    strcpy(ipStatic, p_ipStatic.getValue());
...
bool readConfigFile() {
  ...
  if (!f) {
    ...
    if(json.containsKey("ipStatic")){
      strcpy(ipStatic, json["ipStatic"]);  
    }
